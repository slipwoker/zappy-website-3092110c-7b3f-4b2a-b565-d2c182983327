document.addEventListener('DOMContentLoaded', function () {
  ...
  function validateField(field) {
    ...
    if (value !== '') {
      ...
      if (minLength && value.length < parseInt(minLength, 10)) {
        showFieldError(field, fieldName + ' must be at least ' + minLength + ' characters.');
        return false;
      }

      const maxLength = field.getAttribute('maxlength');
      if (maxLength && value.length > parseInt(maxLength, 10)) {
        showFieldError(field, fieldName + ' must not exceed ' + maxLength + ' characters.');
        return false;
      }
    }
  ...
  if (contactForm) {
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      if (isFormValid) {
        ...
        successMessage.textContent = contactForm.getAttribute('data-success') ||
        ...
        if (firstError) {
          ...
          const fieldTop = firstError.getBoundingClientRect().top + window.pageYOffset;
          window.scrollTo({
            top: fieldTop - navbarH - 20,
            behavior: 'smooth'
          });
        }
      }
  ...
  function injectAnimationStyles() {
    ...
    style.textContent = [
      ...
      '}',
      '[data-animate="slide-left"].' + animationClass + ' {',
      '  transform: translateX(-30px);',
      '}',
      '[data-animate="slide-right"].' + animationClass + ' {',
      '  transform: translateX(30px);',
      '}',
      '[data-animate="slide-left"].' + animationClass + '.' + visibleClass + ',',
      '[data-animate="slide-right"].' + animationClass + '.' + visibleClass + ' {',
      '  transform: translateX(0);',
      '}',
      '[data-animate="zoom-in"].' + animationClass + ' {',
      '  transform: scale(0.9);',
      '}',
      '[data-animate="zoom-in"].' + animationClass + '.' + visibleClass + ' {',
      '  transform: scale(1);',
      '}'
    ].join('\n');
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "דור לדור משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '3092110c-7b3f-4b2a-b565-d2c182983327',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '3092110c-7b3f-4b2a-b565-d2c182983327';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '3092110c-7b3f-4b2a-b565-d2c182983327';
